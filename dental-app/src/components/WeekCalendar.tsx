...
export default function WeekCalendar({
  ...
  function openCreate(day: Date, e: React.MouseEvent<HTMLDivElement>) {
    ...
    const full = toLocalInput(d);
    const date = full.slice(0, 10);
    const start = full.slice(11, 16);
    setModal({ open: true, date, start, end: addMinutesToHHMM(start, 30) });
  }
  ...
  async function submitCreate(fd: FormData) {
    ...
    setSaving(false);
    setModal({ open: false, date: "", start: "", end: "" });
    router.refresh();
  }
    ...
    <div className="space-y-3">
      ...
      {modal.open && (
        <div
          ...
          onClick={() => setModal({ open: false, date: "", start: "", end: "" })}
        >
          <form
            action={submitCreate}
            onClick={(e) => e.stopPropagation()}
